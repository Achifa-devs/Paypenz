...
async function update_pin(req,res) {
    ...
    let pwd = pin.map(item => item).join('')
    let hPwd = await bcrypt.hash(pwd, 10);

    neon_db().then((pool) => {
        pool.query(`
            UPDATE users set pin = '${hPwd}' WHERE user_id = '${user_id}'
        `)
        .then((result) => {
            // const token = createToken(user_id);
            result.rowCount === 1 ? res.status(200).send({bool: true}) : ({bool: false})
        })
        .catch(err => {
            console.log(err)
